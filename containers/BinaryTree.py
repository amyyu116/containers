...
class BinaryTree():
    ...
    def __init__(self, root=None):
        ...
        Note that for an ordinary BinaryTree, we cannot insert more than
        one element in the constructor,
    ...
    def to_list(self, traversal_type):
        ...
        we can always print the returned results if we need to visualize
        them.
        But by returning the results we can also do more computations on
        the results if needed.
        Many of the test cases for more complicated tree functions rely on
    ...
    def __len__helper(node):
        ...
        if a left child exists, add the result of __len__helper on the
        left child;
        if a right child exists, add the result of __len__helper on the
        right child;
    ...
    def height(self):
        ...
        Recall that the height is the maximum length from the root to a
        leaf node.
    ...
    def _height(node):
        ...
        return 1 (for the current node) plus the max of the left and right
        _heights calculated above
        '''
        if node is None:
            return -1
